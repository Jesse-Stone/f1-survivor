...
const DriverProfile = ({driver}) => {
  ...
  return (
    <Stack sx={{ minWidth: 350, backgroundColor: 'white', paddingTop: 1.5, paddingRight: 1, paddingLeft: 1, marginRight:'5px' }}>
      {driver && (
        <>
        <Divider width={'90%'}/>
        <Stack direction={'row'} justifyContent={'space-between'} alignItems={'center'} color="black" sx={{padding:'auto'}}>
          <Stack direction={'row'} alignItems={'center'}>
            <Box sx={{backgroundColor:`${team.color}`, height: '35px', width: '5px'}}></Box>
          <Stack marginLeft={1}>
          ...
          <CardMedia
            ...
            alt={driver.code}
            sx={{ objectFit: 'scale-down', paddingLeft: 1 }}
          />
        </>
      )}
    </Stack>
